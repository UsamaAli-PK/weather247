...
# WeatherAlert will be implemented in alerts app
...
class WeatherAlertEngine:
    ...
    def _should_send_alert(self, user, alert_data) -> bool:
        ...
        try:
            ...
            if not profile or not profile.receive_email_alerts:
                return False
    ...
    def _create_weather_alert(self, user, weather_data, alert_data):
        # WeatherAlert implementation will be moved to alerts app
        pass
    ...
    def _send_email_alert(self, user, alert: WeatherAlert) -> bool:
        ...
        try:
            ...
            logger.info(f"Email alert sent to {user.email} for {alert.city.name}")
            return True
    ...
    def _send_sms_alert(self, user, alert: WeatherAlert) -> bool:
        ...
        try:
            ...
            if response.status_code == 201:
                logger.info(f"SMS alert sent to {profile.phone_number} for {alert.city.name}")
                return True
            else:
...
class AlertMLOptimizer:
    ...
    def get_optimal_alert_threshold(self, user_id: int, alert_type: str) -> float:
        """Get optimized alert threshold for user"""
        try:
            if user_id not in self.user_preferences:
            ...
            # Analyze dismissed alerts to adjust threshold
            dismissed_count = len([
                alert for alert in user_prefs['dismissed_alerts']
                if alert['alert_type'] == alert_type
            ])
            
            acknowledged_count = len([
                alert for alert in user_prefs['acknowledged_alerts']
                if alert['alert_type'] == alert_type
            ])
            
            total_alerts = dismissed_count + acknowledged_count
            
            if total_alerts == 0:
                return 1.0
            
            # If user dismisses many alerts, increase threshold (less sensitive)
            dismissal_rate = dismissed_count / total_alerts
            
            if dismissal_rate > 0.7:
                return 1.3  # Less sensitive
